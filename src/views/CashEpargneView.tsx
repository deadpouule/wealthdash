...
export default function CashEpargneView({ onNavigate, initialTab = 'cash' }: { onNavigate: (v: ViewType) => void, initialTab?: 'cash' | 'epargne' }) {
  const [activeTab, setActiveTab] = useState<'cash' | 'epargne'>(initialTab);
  ...
  const category = activeTab === 'cash' ? 'Cash' : 'Epargne';
  const assets = particulier.listeActifs.filter(a => a.category === category);
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    const newCategoryTotal = updatedActifs.filter(a => a.category === category).reduce((acc, a) => acc + a.value, 0);

    updateParticulier({
      listeActifs: updatedActifs,
      [activeTab]: newCategoryTotal
    });
  ...
  const handleSaveEdit = (e: React.FormEvent, id: string) => {
    ...
    const updatedActifs = particulier.listeActifs.map(a => 
      a.id === id ? { ...a, name: editName, status: editStatus, value: valueNum } : a
    );
    const newCategoryTotal = updatedActifs.filter(a => a.category === category).reduce((acc, a) => acc + a.value, 0);
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-1000 px-6 md:px-10 pt-8 pb-20 w-full max-w-4xl mx-auto">
      ...
      {/* Tabs */}
      <div className="flex p-1 bg-white/[0.02] backdrop-blur-md rounded-2xl border border-white/5 max-w-sm">
        <button
          onClick={() => { setActiveTab('cash'); setShowAdd(false); setEditingId(null); }}
          className={cn(
            "flex-1 py-3 text-sm font-medium tracking-wide transition-all rounded-xl",
            activeTab === 'cash' ? "bg-white/10 text-white shadow-sm" : "text-space-gray hover:text-white/80"
          )}
        >
          Liquidités (Cash)
        </button>
        <button
          onClick={() => { setActiveTab('epargne'); setShowAdd(false); setEditingId(null); }}
          className={cn(
            "flex-1 py-3 text-sm font-medium tracking-wide transition-all rounded-xl",
            activeTab === 'epargne' ? "bg-white/10 text-white shadow-sm" : "text-space-gray hover:text-white/80"
          )}
        >
          Épargne Sécurisée
        </button>
      ...
      <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
        <div className="flex justify-between items-end mb-8">
           <h3 className="text-xl md:text-2xl font-light text-white flex items-center gap-3">
             {activeTab === 'cash' ? <Wallet className="text-[#34C759]" size={24} strokeWidth={1.5} /> : <Landmark className="text-[#34C759]" size={24} strokeWidth={1.5} />}
             {activeTab === 'cash' ? 'Comptes Courants & Cash' : 'Placements & Épargne'}
           </h3>
           <div className="text-right">
             <p className="text-[10px] uppercase tracking-widest text-space-gray mb-1">Total {category}</p>
             <p className="text-2xl font-light text-white tracking-tight">{totalValue.toLocaleString('fr-FR')} MAD</p>
           </div>
        </div>

        {showAdd && (
          <form onSubmit={handleAdd} className="bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-[24px] space-y-4 animate-in fade-in slide-in-from-top-4 mb-8">
             <h3 className="text-white font-medium mb-4 tracking-wide">Ajouter un Compte ({category})</h3>
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
               <input required value={newName} onChange={e=>setNewName(e.target.value)} placeholder="Nom (Ex: BMCE Bank)" className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none font-light transition-colors focus:border-[#34C759]/50" />
               <input required value={newStatus} onChange={e=>setNewStatus(e.target.value)} placeholder="Description (Ex: Compte Courant)" className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none font-light transition-colors focus:border-[#34C759]/50" />
               <input required type="number" step="0.01" value={newValue} onChange={e=>setNewValue(e.target.value)} placeholder="Montant (MAD)" className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none font-light transition-colors focus:border-[#34C759]/50" />
             </div>
             <button type="submit" className="w-full mt-4 bg-[#34C759] text-midnight font-bold py-3 rounded-xl hover:bg-[#34C759]/90 active:scale-[0.98] transition-all">Ajouter</button>
          </form>
        )}
        ...
        <div className="flex flex-col gap-4">
          {assets.length === 0 && !showAdd && (
            <div className="text-center p-12 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[24px]">
               <p className="text-space-gray font-light">Aucun compte {category} enregistré.</p>
            </div>
          )}
             ...
             if (editingId === asset.id) {
               return (
                 <form key={asset.id} onSubmit={(e) => handleSaveEdit(e, asset.id)} className="bg-white/5 backdrop-blur-xl border border-[#34C759] p-6 lg:p-8 rounded-[24px] space-y-4 animate-in fade-in transition-all duration-300 shadow-[0_0_20px_rgba(52,199,89,0.1)]">
                    <h3 className="text-[#34C759] font-medium tracking-wide">Modifier le compte</h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <input required value={editStatus} onChange={e=>setEditStatus(e.target.value)} placeholder="Description" className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none font-light focus:border-[#34C759]/50 transition-colors" />
                      <input required type="number" step="0.01" value={editValue} onChange={e=>setEditValue(e.target.value)} placeholder="Valeur (MAD)" className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none font-light focus:border-[#34C759]/50 transition-colors" />
                    </div>
               ...
               <div key={asset.id} className="bg-white/5 backdrop-blur-xl border border-white/10 p-6 md:p-8 rounded-[24px] group hover:bg-white/10 hover:border-[#34C759]/50 transition-all cursor-default relative overflow-hidden flex flex-col justify-between active:scale-[0.98]">
                 <div className="flex flex-col md:flex-row justify-between items-start md:items-center relative z-10 gap-4 md:gap-0">
                   <div>
                     ...
                     <p className="text-xs text-space-gray tracking-wide">{asset.status}</p>
                   </div>
      ...
      {!showAdd && (
        <div className="flex justify-center mt-12 pb-10">
          <button onClick={() => setShowAdd(true)} className="flex items-center justify-center gap-3 w-full max-w-md py-4 px-6 rounded-[20px] bg-white/5 backdrop-blur-xl border border-[#34C759]/30 hover:border-[#34C759] shadow-[inset_0_1px_4px_rgba(255,255,255,0.02),0_0_15px_rgba(52,199,89,0.1)] hover:shadow-[0_0_25px_rgba(52,199,89,0.25)] transition-all duration-500 group active:scale-[0.98]">
            ...
            <span className="text-white font-bold tracking-wide uppercase text-sm">Ajouter {activeTab === 'cash' ? 'un compte cash' : 'une épargne'}</span>
          </button>
        </div>
      )}
    </div>
